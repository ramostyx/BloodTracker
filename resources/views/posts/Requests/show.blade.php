<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto flex flex-col gap-4 justify-start ">
        ...
        <div>
            <h5 class="font-semibold">{{$request->post->bloodtransfercenter->user->name}}</h5>
        </div>
    </div>
    <div class="bg-red-500 px-3 relative py-2 text-white rounded-lg">
        <div class="flex justify-start items-center gap-2">
            <h2 class="text-xl font-bold">{{$request->post->title}}</h2>
            <div class="flex justify-start items-center gap-0.5">
                <ion-icon name="location-outline"></ion-icon>
                <h5 class="text-sm font-semibold">province</h5>
                {{--                <h5 class="text-sm font-semibold">{{$request->post->bloodtransfercenter->user->address->province}}</h5>--}}
            </div>
        </div>
        <h5>urgency: <span>{{$request->urgency}}</span></h5>
        <p>
            {{$request->post->body}}
        </p>
    </div>
    ...
    <div class="flex justify-start items-center gap-1">
        <span>Required blood types: </span>
        <div class="flex justify-start items-center">
            @foreach($request->requiredBloodTypes as $bloodType)
                <x-blood-bag Type="{{$bloodType}}"/>
            @endforeach
        </div>
    </div>
    ...
    <div class="bg-white border-2 border-red-500 rounded-lg p-4 ">
        @auth
            <form action="{{route('comment.make',$request->post->id)}}" method="POST" class="flex gap-2">
                @csrf

                <div>
                    <div class="w-12 h-12 rounded-full object-cover"
                         style="background-image: url("{{asset(\Illuminate\Support\Facades\Auth::user()->profileImage)}}")"></div>
                </div>
    ...
    <div class="collapse show" id="commentcollapse">
        @foreach($request->post->comments->sortByDesc('created_at') as $comment)
                            ...
                            <form action="{{route('comment.destroy',$comment->id)}}" method="POST">
                                ...
                                <button class="bg-none">
                                    <ion-icon name="trash-outline"
                                              class="p-1.5 text-md hover:bg-red-100 hover:text-red-400 rounded-lg"></ion-icon>
                                </button>
                            </form>
                        </div>
                        @endOwner
